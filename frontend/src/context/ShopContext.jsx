...
const ShopContextProvider = (props) => {
    ...
    const updateQuantity = async (itemId, model, quantity) => {
        let cartData = structuredClone(cartItems);
        cartData[itemId][model] = quantity;
        setCartItems(cartData);

        if (token) {
            try {
                await axios.post(backendUrl + '/api/cart/update', { itemId, model, quantity }, { headers: { token } });
            } catch (error) {
                console.log(error);
                toast.error(error.message);
            }
